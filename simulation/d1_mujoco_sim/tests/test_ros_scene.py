...
def test_object_mesh_markers_use_original_assets_and_runtime_body_poses(
    ...
    specs = object_mesh_specs(model, data, ROOT / "simulation" / "objects")
...
def test_object_mesh_markers_follow_free_body_motion(
    ...
    mujoco.mj_forward(model, data)
    cube = object_mesh_specs(model, data, ROOT / "simulation" / "objects")[0]
...
def test_rviz_layer_defaults_keep_meshes_on_and_collisions_off() -> None:
    rviz = yaml.safe_load(
        (ROOT / "src" / "d1_moveit_config" / "config" / "moveit.rviz").read_text()
    )
    displays = {
