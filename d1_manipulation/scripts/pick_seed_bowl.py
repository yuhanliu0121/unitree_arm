#!/usr/bin/env python3
"""Send the fixed-scene bowl hint to the staged PickObject Action."""
...
def receive_bowl(port: int, timeout_s: float):
    ...
    try:
        while time.monotonic() < deadline:
            ...
            for line in sock.recv(4096).decode("ascii").splitlines()[1:]:
                fields = line.split()
                if len(fields) == 8 and fields[0] == "bowl":
                    return tuple(float(value) for value in fields[1:4])
    finally:
        sock.close()
    raise TimeoutError("no bowl scene truth received")
...
def main() -> int:
    ...
    target = receive_bowl(args.port, args.timeout)
    initial_z = target[2]
    rclpy.init()
    node = rclpy.create_node("d1_pick_seed_bowl")
    ...
    try:
        ...
        print(
            ...
            f"rim_azimuth={result.grasp_yaw_degrees:.1f} deg"
        )
        if args.stage in ("lift", "carry"):
            final = receive_bowl(args.port, args.timeout)
            rise = final[2] - initial_z
            if rise < 0.05:
                print(f"PICK FAILED: bowl rose only {rise:.3f} m")
                return 1
            print(f"PHYSICAL LIFT VERIFIED: bowl rise={rise:.3f} m")
        point = result.estimated_center.point
        print(
            f"PICK STAGE SUCCEEDED: {args.stage} class={result.class_name} "
            f"bottom=({point.x:.3f},{point.y:.3f},{point.z:.3f})"
        )
        return 0
    finally:
        node.destroy_node()
