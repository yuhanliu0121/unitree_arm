...
public:
  ...
  : node_(node), runtime_(runtime)
  {
    ...
    finetune_min_improvement_ = parameterOrDeclare(
      node_, "zucchini_finetune_min_improvement_ratio", 0.30);
    ...
    if (finetune_closing_axis_camera_.norm() < 0.9 ||
      ...
      finetune_max_stddev_ <= 0.0 || finetune_min_improvement_ < 0.0 ||
      finetune_min_improvement_ >= 1.0 || finetune_settle_s_ < 0.0)
    {
  ...
  bool fineTune(PreparedPick& plan, StrategyFailure& failure) override
  {
    ...
    for (int correction = 0; correction <= finetune_max_corrections_; ++correction) {
        ...
        outside_error > (1.0 - finetune_min_improvement_) * previous_outside_error)
      {
        std::ostringstream detail;
        detail << std::fixed << std::setprecision(1)
               << "visual correction did not reduce zucchini safe-slab outside error by "
