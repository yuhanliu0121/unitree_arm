...
struct Config
{
  ...
  std::string native_segment_topic{"d1_native_joint_segment"};
...
Config parse_arguments(int argc, char ** argv)
{
  ...
  for (int index = 1; index < argc; ++index)
  {
    ...
    {
      std::cout
        << "Usage: d1_control_gateway [--domain N] [--interface NAME] "
        << "[--direction both|command|feedback|status] "
        << "[--command-topic TOPIC] [--native-segment-topic TOPIC] "
        << "[--feedback-topic TOPIC] [--status-topic TOPIC] "
    ...
    else if (option == "--native-segment-topic")
    {
      config.native_segment_topic = value;
    }
...
public:
  ...
  bool start()
  {
    ...
    if (socket_fd_ < 0)
    {
      std::cerr << "d1_control_gateway: socket failed: " << std::strerror(errno) << '\n';
      return false;
    }
    ...
    if (command_enabled(config_))
    {
      ...
      {
        std::cerr << "d1_control_gateway: bind 127.0.0.1:" << config_.command_port
    ...
    try
    {
      ...
      if (command_enabled(config_))
      {
        ...
        segment_publisher_ = std::make_unique<Publisher>(config_.native_segment_topic);
        segment_publisher_->InitChannel();
      }
    }
    catch (const std::exception & error)
    {
      std::cerr << "d1_control_gateway: DDS setup failed: " << error.what() << '\n';
      stop();
      return false;
    }
    ...
    if (command_enabled(config_))
    {
      std::cout << " command=" << config_.command_topic
                << " native_segment=" << config_.native_segment_topic;
      std::cout
  ...
  int run()
  {
    ...
    while (!g_stop_requested)
    {
      ...
      if (result < 0)
      {
        ...
        std::cerr << "d1_control_gateway: poll failed: " << std::strerror(errno) << '\n';
        return 1;
      }
...
private:
  ...
  {
    std::optional<JointPacket> latest_command;
    while (true)
    {
      JointPacket packet;
      ...
      if (received < 0)
      {
        ...
        {
          std::cerr << "d1_control_gateway: receive failed: "
                    << std::strerror(errno) << '\n';
        }
        break;
      }
      if (received != static_cast<ssize_t>(sizeof(packet)) ||
          !d1_control::packet_header_is_valid(packet))
      {
        std::cerr << "d1_control_gateway: ignored invalid local command packet\n";
        continue;
      }
      ...
      {
        std::cerr << "d1_control_gateway: ignored unsupported local command kind\n";
        continue;
      }
      bool finite = true;
      ...
      const bool native_segment = packet.smoothing_mode == 2U ||
        packet.smoothing_mode == 3U;
      const bool valid_segment_profile = !native_segment ||
        (packet.duration_ms > 0U && packet.duration_ms <= 65535U &&
        packet.acceleration_ms <= 65535U && packet.deceleration_ms <= 65535U &&
        packet.acceleration_ms + packet.deceleration_ms <= packet.duration_ms);
      if (!finite || packet.smoothing_mode > 3U || !valid_segment_profile ||
          (!native_segment && packet.duration_ms > static_cast<std::uint32_t>(
            std::numeric_limits<std::int16_t>::max())))
      {
        std::cerr << "d1_control_gateway: ignored unsafe local command packet\n";
        continue;
      }
      latest_command = packet;
    }
    if (latest_command)
    {
      publish_native_command(*latest_command);
    }
  }
  ...
  void publish_native_command(const JointPacket & packet)
  {
    ...
    if (packet.smoothing_mode == 2U || packet.smoothing_mode == 3U)
    {
      publish_native_segment(packet, native_sequence);
      return;
    }
    ...
    ++command_count_;
    std::cout << "d1_control_gateway: forwarding command #" << command_count_
    ...
    {
      std::cerr << "d1_control_gateway: native DDS command publish failed"
                << " native_seq=" << native_sequence << std::endl;
      return;
    }
    std::cout << "d1_control_gateway: DDS Write accepted native_seq="
              << native_sequence << std::endl;
  }

  void publish_native_segment(
    const JointPacket & packet, const std::uint32_t native_sequence)
  {
    if (!segment_publisher_)
    {
      std::cerr << "d1_control_gateway: joint segment publisher is unavailable\n";
      return;
    }
    std::ostringstream payload;
    payload.setf(std::ios::fixed);
    payload.precision(7);
    payload << "{\"seq\":" << native_sequence
            << ",\"motion_profile\":" << packet.smoothing_mode
            << ",\"duration_ms\":" << packet.duration_ms
            << ",\"acceleration_ms\":" << packet.acceleration_ms
            << ",\"deceleration_ms\":" << packet.deceleration_ms
            << ",\"angles_deg\":[";
    for (std::size_t index = 0; index < kD1JointCount; ++index)
    {
      if (index > 0U) payload << ',';
      payload << packet.angle_deg[index];
    }
    payload << "]}";
    ArmString message;
    message.data_() = payload.str();
    const bool accepted = segment_publisher_->Write(message);
    ++command_count_;
    std::ostream & stream = accepted ? std::cout : std::cerr;
    stream << "d1_control_gateway: joint segment #" << command_count_
           << " local_seq=" << packet.sequence
           << " native_seq=" << native_sequence
           << " profile=" << (packet.smoothing_mode == 3U ?
             "uniform_joint_speed" : "common_arrival")
           << " duration_ms=" << packet.duration_ms
           << " accel_ms=" << packet.acceleration_ms
           << " decel_ms=" << packet.deceleration_ms
    ...
    const int funcode, const char * data, const std::uint64_t local_sequence)
  {
    ...
    payload << "{\"seq\":" << native_sequence
            ...
            << ",\"data\":{" << data << "}}";
    std::cout << "d1_control_gateway: forwarding management command"
    ...
    if (!publisher_->Write(message))
    {
      std::cerr << "d1_control_gateway: native DDS management publish failed"
                << " native_seq=" << native_sequence << std::endl;
      return;
    }
    std::cout << "d1_control_gateway: DDS management Write accepted native_seq="
  ...
  void handle_status(const void * raw_message)
  {
        ...
        address == 3 && (function == 1 || function == 2))
    {
      ...
      {
        std::cout << "d1_control_gateway: D1 "
      ...
      {
        std::cerr << "d1_control_gateway: malformed D1 ACK payload="
  ...
  void stop()
  {
    ...
    if (segment_publisher_)
    {
      segment_publisher_->CloseChannel();
      segment_publisher_.reset();
    }
    if (dds_initialized_)
    {
  ...
  std::unique_ptr<Publisher> segment_publisher_;
...
int main(int argc, char ** argv)
{
  ...
  {
    std::cerr << "d1_control_gateway: " << error.what() << '\n';
    return 2;
  }
}
