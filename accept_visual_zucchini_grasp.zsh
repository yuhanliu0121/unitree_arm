...
function usage() {
  print 'Usage: ./accept_visual_zucchini_grasp.zsh [--headless] [--rviz] [--object zucchini|bowl] [--stage compute|pregrasp|descend|lift|carry]'
}
...
if [[ ${object} != zucchini && ${object} != bowl ]]; then
...
mkdir -p ${ROS_LOG_DIR}
run_dir=$(mktemp -d /tmp/d1_visual_zucchini.XXXXXX)
...
if [[ ${rviz} == true && -t 0 ]]; then
  print -n "Initial scene ready. Press Enter to run staged visual ${object} grasp..."
  IFS= read -r reply
fi
...
if (( goal_status == 0 )) && grep -q 'PICK STAGE SUCCEEDED' ${goal_log}; then
  if [[ -d /tmp/d1_zucchini_debug_latest ]]; then
    mkdir -p ${run_dir}/geometry
    cp -a /tmp/d1_zucchini_debug_latest/. ${run_dir}/geometry/
  fi
  print
  print "ACCEPTANCE PASSED: visual ${object} stage ${stage}"
  print "Logs: ${run_dir}"
  if [[ ${rviz} == true && -t 0 ]]; then
